...
void EscortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, ObjectGuid playerGUID /* = 0 */, Quest const* quest /* = nullptr */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    {
        TC_LOG_ERROR("scripts.ai.escortai", "EscortAI::Start: (script: %s) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u) (%s)", me->GetScriptName().c_str(), quest ? quest->GetQuestId() : 0, me->GetGUID().ToString().c_str());
        return;
    }
    ...
    if (_returnToStart && _instantRespawn)
        TC_LOG_DEBUG("scripts.ai.escortai", "EscortAI::Start: (script: %s) is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn (%s)", me->GetScriptName().c_str(), me->GetGUID().ToString().c_str());
